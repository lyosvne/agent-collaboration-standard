...
title: "架构真值"
signoff: "用户 2026-08-12（Pi 运行、六角色同步与 ZCode 评审执行隔离）"
---
# 智能体矩阵架构真值 v1.2
...
|---|---|---|---|---|
| **ZCode** | 评审优先执行/分析/反哺 | ❌ 不承担中央调度 | 通过结构化 handoff 接收评审或实现任务 | ✅ 本地 shell/独立分支/PR；❌ 自批/master/SSH/部署/生产/secrets |
...
- **ZCode**：按需接收评审或实现任务；不承担中央调度，且实现结果必须独立终审
