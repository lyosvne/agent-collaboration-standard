...
> 裁定: 用户
> 日期: 2026-07-25（v1.1）→ 2026-08-01（v1.17）→ 2026-08-08（v1.18）
> 性质: 路线图罗盘——定义方向和退出条件，不定义具体实施（实施由当前阶段智能体分解执行）
> 依据: 北极星 v1.4 + Aetheris 蓝图 v1.11 + soul.yaml + 全量资产调研 + 86 号治理规格 `specs/pi-cognitive-plane-and-self-evolution-v1.0.md`。86 号是对 Aetheris 当前产品定位的有效战略覆盖，Aetheris 仓内投影另行同步。
...
服务林于炜的长期工作与成长：通过 Pi 思维平面、自进化闭环和亲属矩阵，把信息、判断、任务与反馈持续转化为可验证的系统能力。飞书 CSM 是重要业务背景和历史资产，不再等同于 Aetheris 产品本体。
...
| **系统自治** | 系统不依赖你推进工作的程度 + 运转结果反哺系统变强的程度 | Pi ECS 已运行采集→反思→记忆→日报闭环；跨智能体执行与能力晋级仍在治理 | 24h按轨道运转（T3红线内）+ 执行结果自动沉淀为新能力 |
| **评估反馈** | 系统每个产出是否有验证证据 + 你的反馈被系统吸收沉淀 | 已有记忆沉淀和质量门基线；反馈到行为变化的覆盖率仍待量化 | 每次产出附验证证据、你的反馈被吸收沉淀 |
...
用户的稳定身份始终是**战略制定者、T3 决策者和最终裁判**，日常认知运转由 Pi 自闭环承担，需要多 agent 协作时由用户即席指派。下面的”阶段介入方式”只描述用户在不同成熟阶段需要投入多少，不重新定义用户身份。
...
**方向**：你交给系统的任务，它闭环交付。Aetheris（Pi 的思维平面）产出你能用的真实结果。（v1.17 修订，因 86 号生效：Aetheris 定位从 CSM 产品本体改为 Pi 的思维平面）

**阶段介入方式**：发起任务并抽样评审，系统标红时介入

**退出条件（KR）**：
- ~~你能通过Aetheris跟进至少1个真实客户，数据是准的（W5.5数据流闭环）~~ → **v1.17 修订**：Aetheris 作为 Pi 思维平面，能让你看见“Pi 怎么想、怎么看我的工作、选什么方向”，并通过反馈引导 Pi 成长（三形式：文字日记/HTML 活看板/提案，见 `specs/pi-cognitive-plane-and-self-evolution-v1.0.md`）
- ~~3类标准任务（客户跟进/知识沉淀/内容生成）连续5次介入≤2次~~ → **v1.17 修订**：Pi 的认知自更新闭环跑通——观察→假设→确认三级分级落地，反思产出按主题维度归类，行动建议落在用户能力边界内（同上规格）
- **CSM 产品功能冻结**（v1.17 新增）：蓝图原 M08 CSM 工作流引擎等 CSM 产品功能 **冻结**（代码数据保留，写明解冻条件），不投入新资源。如未来需要 CSM 功能，另议并解冻。
- **降级路径**：产出质量不达标时可一键退回手动模式
- **你的验收**：你在思维平面上看到的 Pi 思考外显，是否真的帮你理解了 Pi、并能让你的反馈被 Pi 吸收（非 CSM 数据准确度验收）

### O3：工具箱协作

**方向**：你即席指派工具箱 agent 时，协作链路顺畅，不靠你在 agent 间手动转发。

**阶段介入方式**：以观测为主，偏航时矫正

**退出条件（KR）**：
- 多agent协作产出你验收通过的方案（你指派一个任务经多agent交接→互检→评审→交付）
- 你指派协作时不需要在agent间手动转发信息（从"消息总线"退场）
- Pi 自动采集七个维度的水位数据，事件驱动给你校准报告
- Pi 能主动推送需介入信号（异常/偏航/不可委托事项）——这是你进入观测员模式的前置
- **降级路径**：工具箱协作出错时可一键退回你手动中转
- **你的验收**：你确认工具箱协作的质量达到你的标准

### O4：自治运转

**方向**：Pi 按轨道自转，持续服务你演进的需求。

**阶段介入方式**：只定战略、T3 决策和最终满意度，偶尔校准

**退出条件（KR）**：
- 你只需说战略方向，Pi 理解并自主完成或建议你即席指派（M19认知转译成熟）
- Pi 自转中持续进化（执行结果反哺为新能力，有质量门把关）
...
**反馈吸收依赖记忆层**：Pi 已有 `memoryProvider` 持久化与 embedding 检索基线；反馈从“被记住”晋级为“改变行为或能力”仍须经过认知质量门或能力质量门。OpenViking 等知识层扩展不是当前闭环存在与否的前提。
...
| O2 执行闭环 | Pi 认知平面 + 记忆/反馈闭环 + Wave2-3（知识/编排器/Mira） | CSM W5.5 已冻结，不作为退出条件 |
...
**Pi ECS 已运行自闭环（采集→反思→记忆→飞书消息）；治理真源已同步（mirror `549cb49`，自动同步工作流 2026-08-09 验收）；Pi 定位已修订为自闭环认知系统（北极星 v1.5，取消中央协调者角色，工具箱即席指派）。当前重点是 Pi 自进化质量门落地（认知自更新分级 + 能力自改提案）和下游治理文件措辞同步。CSM 产品功能保持冻结。（2026-08-12 更新）**

### 已完成（O1 协作底座 + 退役清理 + 治理真值）
- CC→ZCode 迁移（CC 完全退役 + 密钥清除本机零残留）
- Pi ECS 基线闭环已运行；后续只跟踪具体治理增强和运行可靠性，不再把“部署 Pi”列为待办
...
7. ~~**scripts/ 工具脚本长期维护归属**（5 个 .py 无 spec；Phase D-B 已触及，建议合并处理）~~ ✅ **已闭环（2026-07-28）**：
...
### O2 并行推进中
- Pi 认知平面、记忆/反馈闭环和亲属矩阵协作
- CSM W5.5 数据流仅保留代码与历史证据，不投入新资源

---

## 分歧项（已裁定）

### W5.5 数据流闭环归属 → 冻结历史资产（v1.18 更新）

- W5.5 的代码、数据和生产证据保留，用于审计和未来可能的解冻评估。
- W5.5 不再是 O1/O2 退出条件，也不进入当前资源投入队列。
- 只有用户明确解冻 CSM 后，才重新定义 owner、验收标准和生产授权。
...
- 不恢复独立 Solo。统一 Trae 是实现、集成、Git/PR、产品测试和浏览器验收的当前承载者；历史 `agent/solo` 只保留审计价值。

---

## 版本历史

> 以下旧版本行保留当时角色和方案原文，只用于审计；当前角色与任务路由以 v1.18 正文为准。

| 版本 | 日期 | 变更 |
|---|---|---|
| v1.18 Pi 运行与六角色同步 | 2026-08-08 | 用户确认 Pi 已在 ECS 运行自进化闭环；统一用户稳定身份与 Pi 中央协调边界；统一 Trae 并退役独立 Solo；ZCode 改为非终端评审/反哺；CSM W5.5 从当前阶段和退出条件移除；新增 86 号可执行治理规格与 canonical 版本契约。 |
| v1.17 O2 退出条件修订（86号生效） | 2026-08-01 | **因 86 号提案（Pi 认知平面与 Aetheris 思维平面）生效修订**：O2 方向从"Aetheris 作为 CSM 产品闭环交付"改为"Aetheris 作为 Pi 思维平面外显思考+反馈引导"；O2 退出条件中原 CSM 数据流/客户跟进 KR 替换为思维平面认知闭环 KR（三形式/三级分级/主题归类/能力边界）；新增 CSM 产品功能冻结条款（M08 等冻结，代码保留写解冻条件）。依据北极星 v1.3 + 86 号 §2.1/§2.3/§4.5。 |
