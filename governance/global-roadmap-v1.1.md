...
status: "active"
type: "roadmap"
supersedes: "global-roadmap-v1.16"
title: "全局路线图"
signoff: "ZCode+Qoder 2026-07-25（v1.1）；用户 2026-08-01（v1.17，因 86 号生效修订 O2）"
---
# 全局路线图 v1.17（修订稿）

> 签发: ZCode + Qoder（共创，经两轮交叉对抗式论证，19条逐条修订）
> 裁定: 用户
> 日期: 2026-07-25（v1.1）→ 2026-08-01（v1.17）
> 性质: 路线图罗盘——定义方向和退出条件，不定义具体实施（实施由当前阶段智能体分解执行）
> 依据: 北极星v1.3 + Aetheris蓝图v2.0 + soul.yaml + Codex知识库战略洞察 + 全量资产调研 + Qoder客户端两轮审查 + 86号提案（Pi认知平面与Aetheris思维平面）
...
**退出条件（KR）**：
- ~~你能通过Aetheris跟进至少1个真实客户，数据是准的（W5.5数据流闭环）~~ → **v1.17 修订**：Aetheris 作为 Pi 思维平面，能让你看见"Pi 怎么想、怎么看我的工作、选什么方向"，并通过反馈引导 Pi 成长（三形式：文字日记/HTML 活看板/提案，见 86 号 §2.3）
- ~~3类标准任务（客户跟进/知识沉淀/内容生成）连续5次介入≤2次~~ → **v1.17 修订**：Pi 的认知自更新闭环跑通——观察→假设→确认三级分级落地，反思产出按主题维度归类，行动建议落在你的能力边界内（86 号 §4.5 B 组规格要求）
