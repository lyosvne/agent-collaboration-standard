...
## v3.3 → v3.4 改动（极小，2 行）
1. 门禁 4 grep 修复：`grep -E "退役\|retire..."` → `grep -E "退役|retire..."`（去反斜杠，ERE 正确语法）
2. 路径硬编码修复：`/c/Users/Admin/.agent-collaboration/` → `"$HOME/.agent-collaboration/"`

## v3.3 三方结论（背景）
- A: ❌ 门禁 4 fail-open（本次修复点）
- B: ✅ 通过（v3.3 已放行，v3.4 不动 B 的部分）
- C: ❌ 同 A（+ 备注 1 路径硬编码，本次修复）

## 仅核对此 2 行修订是否完整闭环 v3.3 阻断
...
updated: 2026-07-26
owner: ZCode
title: ~/.agent-collaboration → git 仓库 同步方案 v3.4（修订版）
...
revision_basis: "节点1五轮复核（B 通过；A/C 发现门禁4 grep -E 反斜杠 fail-open）"
supersedes:
  - "v3.3 / v3.2 / v3.1 / v3.0 / v2.0 / v1.0 (历史版本)"
---

# 同步方案 v3.4（修订版）

## 〇、v3.3 → v3.4 修订（响应 A/C v3.3 发现）

v3.3 B 终于放行（3 真 bug 闭环），但 A/C 发现 B.5 门禁 4 的 grep 引入新 fail-open：

| A/C 发现 | v3.4 修订 |
|---|---|
| 门禁 4 `grep -E "退役\|retire\|..."` 的 `\|` 在 ERE 下是字面竖线不是"或"，HISTORY_KEYS 永远空 → comm 永远输出空 → 门禁永远"通过" | 改为 `grep -E "退役|retire|..."`（去掉反斜杠，ERE 正确语法）|
| C 备注 1：硬编码 `/c/Users/Admin/` 机器绑定 | 改用 `$HOME/.agent-collaboration/` |

**v3.4 改动量极小（2 行）**，B 已放行的部分不动。预期三方一致通过。
...
v3.3 修订：删除 v3.2 的 `tr -d ' '`（会把 "Claude Code" 变 "ClaudeCode" 导致 case 失效，B.3 真 bug），改用更稳健的 awk 列提取。

```bash
# 读取例外清单，只替换标注为 ROLE 的行
# 用 awk 按字段位置提取（不破坏空格）
EXCEPTIONS=~/.agent-collaboration/archive/retired-terms-exceptions-20260726.md

awk -F'|' 'NR>2 && $5 ~ /ROLE/ {print $2"|"$3"|"$4"|"$5}' "$EXCEPTIONS" | \
while IFS='|' read -r file_raw line_raw tool_raw tag_raw; do
    # 去除字段前后的空白（不删工具名内部的空格）
    file=$(echo "$file_raw" | sed 's/^[[:space:]]*//; s/[[:space:]]*$//')
    line=$(echo "$line_raw" | sed 's/^[[:space:]]*//; s/[[:space:]]*$//')
    tool=$(echo "$tool_raw" | sed 's/^[[:space:]]*//; s/[[:space:]]*$//')

    # 按 tool 名映射占位符（保留原始空格）
    case "$tool" in
        ...
        *) echo "⚠️ 未知工具: $tool，跳过"; continue ;;
    esac

    # 替换指定行（用 | 作为 sed 分隔符，避免文件路径 / 冲突）
    target="$HOME/.agent-collaboration/$file"
    [ -f "$target" ] && sed -i "${line}s|${tool}|${placeholder}|g" "$target"
done
```

**A.5 验证（机器可判，对应 B 的门禁 3/4）**

v3.3 修订：门禁 4 从"打印数量"改为"硬门禁"（不一致直接 exit 1），并补齐 v3.2 遗漏的 `claude-zhipu`。

```bash
# 门禁 3: 现行规范中退役角色引用 = 0（v3.3 补齐 claude-zhipu）
REMAINING_ROLE=$(grep -rn "Claude Code\|claude-zhipu\|Codex\|QoderWork\|Trae IDE" ~/.agent-collaboration/standards/ 2>/dev/null | \
    grep -v "\[RETIRED-\|退役\|retire\|历史\|归档\|已删" | head -1)
[ -n "$REMAINING_ROLE" ] && { echo "❌ 现行角色引用未清零: $REMAINING_ROLE"; exit 1; }
echo "✅ 门禁 3: 现行角色引用 0"

# 门禁 4: 历史引用 100% 命中例外清单（v3.3 改为硬门禁）
# 思路：每个历史引用必须在例外清单中有对应记录（文件+行号+工具名）
EXCEPTIONS=~/.agent-collaboration/archive/retired-terms-exceptions-20260726.md

# 收集所有历史引用的键（文件+行号+工具）
# v3.4 修复：grep -E 下 \| 是字面竖线不是"或"，改为无反斜杠（A/C v3.3 发现的 fail-open bug）
HISTORY_KEYS_FILE=$(mktemp)
grep -rn "Claude Code\|claude-zhipu\|Codex\|QoderWork\|Trae IDE" ~/.agent-collaboration/standards/ 2>/dev/null | \
    grep -E "退役|retire|历史|归档|已删" | \
    awk -F: '{
        # 提取工具名（简化：检查每个工具是否在行内）
        line=$0
        for (t in arr) {}
        if (line ~ /Claude Code/) print $1":"$2"|Claude Code"
        if (line ~ /claude-zhipu/) print $1":"$2"|claude-zhipu"
        if (line ~ /Codex/) print $1":"$2"|Codex"
        if (line ~ /QoderWork/) print $1":"$2"|QoderWork"
        if (line ~ /Trae IDE/) print $1":"$2"|Trae IDE"
    }' > "$HISTORY_KEYS_FILE"

# 收集例外清单的键
EXCEPTION_KEYS_FILE=$(mktemp)
awk -F'|' 'NR>2 && $5 ~ /HISTORY/ {
    file=$2; line=$3; tool=$4
    gsub(/^[[:space:]]+|[[:space:]]+$/, "", file)
    gsub(/^[[:space:]]+|[[:space:]]+$/, "", line)
    gsub(/^[[:space:]]+|[[:space:]]+$/, "", tool)
    # 转换为完整路径（v3.4：用 $HOME 而非硬编码，消除机器绑定，C v3.3 备注 1）
    print "'"$HOME"'/.agent-collaboration/"file":"line"|"tool
}' "$EXCEPTIONS" > "$EXCEPTION_KEYS_FILE"

# 比对：HISTORY_KEYS 必须全部在 EXCEPTION_KEYS 中
MISSING=$(comm -23 <(sort "$HISTORY_KEYS_FILE") <(sort "$EXCEPTION_KEYS_FILE") | head -1)
rm -f "$HISTORY_KEYS_FILE" "$EXCEPTION_KEYS_FILE"

if [ -n "$MISSING" ]; then
    echo "❌ 门禁 4 失败：历史引用未命中例外清单:"
    echo "$MISSING"
    exit 1
fi
echo "✅ 门禁 4: 历史引用 100% 命中例外清单"
```
...
# v3.3 修订：处理 git grep 退出码（0=命中/1=正常无命中/>1=扫描失败），避免 fail-open
set +e  # 临时关闭 errexit，手动处理 git grep 退出码
SECRET_HITS=0
SCAN_ERRORS=0
while IFS= read -r pattern; do
    [ -z "$pattern" ] && continue
    # -e 防止 pattern 以 - 开头被当选项
    git grep --cached -l -F -e "$pattern" > /tmp/gg-out 2>/tmp/gg-err
    rc=$?
    case $rc in
        0)   # 有命中
            hits=$(cat /tmp/gg-out)
        ...
        1)   # 正常无命中
            :
            ;;
        *)   # 扫描失败（rc>1），必须阻断，不能当零命中放行
            echo "❌ git grep 扫描失败 (rc=$rc): $(cat /tmp/gg-err)"
            SCAN_ERRORS=$((SCAN_ERRORS+1))
            ;;
    esac
done <<< "$VALID_PATTERNS"
rm -f /tmp/gg-out /tmp/gg-err
set -e

if [ "$SCAN_ERRORS" -gt 0 ]; then
    echo "❌ 扫描本身失败 $SCAN_ERRORS 次（不是无命中，是异常），已 reset 暂存区"
    git reset
    exit 1
fi
